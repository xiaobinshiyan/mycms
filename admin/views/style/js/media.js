...
//编辑状态
function status_edit(state,id,url)
{
	if(typeof(id) !== 'number'){
			$.dialog({
				message : "请选择有效信息",
...
//保存表单
function save_form(tab,url)
  {
  	var tab = 'form[name='+tab+']';
  	var postdata = $(tab).serialize();
	$.ajax({
			url: url,
			type: 'POST',
			dataType: 'JSON',
			...
			if(!data.status){
				...
				// alert(data.tid);
				$("#content_en input[name='id']").val(data.tid);
				$("#content_ch input[name='id']").val(data.tid);
				$.dialog({
					message : data.message,
					timeout : data.timeout || 1,
					type : "success"
