...
function flush_left_menu(nid)
{
    ...
    $.ajax({
        type: "GET",
        url: back_url+'/welcome/get_child_menu',
        data: {pid: nid},
        cache: false,
...
function show_iframe(nid) {
    ...
    if (menu_cache.iframe[nid]) {
        ...
        var html = '<iframe nid="' + nid + '" src="' + url + "/?rnd=" + Math.random() + '" scrolling="auto" frameborder="0" style="height: 100%;width: 100%;"></iframe>';
...
function add_menu_history(nid, title) {
    ...
    if ($("div.favorite_menu a[nid='" + nid + "']").length == 0) {
        ...
        html += "<a class='menu' nid='" + nid + "'>" + title + "</a>";
